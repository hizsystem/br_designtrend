# 오늘의 디자인 트렌드 (회사판) — 발행 지침

이 저장소는 매일 디자인 트렌드 리포트를 발행하는 **정적 사이트**입니다(일반 대중 대상).
임무: **오늘자 리포트를 만들어** 사이트에 추가하는 것. 모든 본문은 **한국어**로 작성한다.

## ⭐ 톤 규칙 (이 회사판의 핵심)
- **일반인도 이해하기 쉬운 친절한 말투/용어**로 쓴다. 디자이너 전문용어는 피하거나 **쉬운 말로 풀어서** 설명한다.
  - 예: "그로테스크" → "깔끔한 고딕 글씨체", "글래스모피즘" → "반투명 유리 효과", "벤토 그리드" → "도시락처럼 칸 나눈 배치".
- 문장은 짧고 친근하게. 어려운 영어 용어엔 괄호로 쉬운 설명을 덧붙인다.
- 메인/하이라이트 컬러는 **파란색**(`--accent: #4196EC`, 보조 `#364467`) 팔레트를 유지한다.
- 카테고리 이름도 친절하게: 글씨(폰트) 트렌드 / 이미지·그래픽 트렌드 / 웹·앱 화면 트렌드 / 색 조합 트렌드 / 카드뉴스 트렌드.

> 참고: 이 회사판은 아직 전용 GitHub 저장소가 없다. 호스팅·자동발행은 별도로 설정 예정이므로,
> 아래 절차의 "커밋·푸시" 단계는 저장소가 준비된 뒤에 적용한다.
